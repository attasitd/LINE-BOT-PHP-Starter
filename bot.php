...
if (!is_null($events['events'])) {
	...
	foreach ($events['events'] as $event) {
		...
		if ($event['type'] == 'message' && $event['message']['type'] == 'text') {
			...
			$text2 =  "ยินดีต้อนรับเข้าสู่ระบบแจ้งเตือนงานสารบรรณวิทยาลัยการอาชีพนวมินทราชินีแม่ฮ่องสอน [rms.nawamin.ac.th]";
			...
			if (strlen($text)==13){
				...
				if ($data_a == "OK"){
					...
					$text = $text2 . '  หมายเลขลงทะเบียนของท่านเคยลงทะเบียนแล้ว <กรุณาติดต่อผู้ดูแลระบบ [Attasit Datsong:HTTP-Dup]>';
				}else{
